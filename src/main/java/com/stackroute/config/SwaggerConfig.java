...
public class SwaggerConfig {
    ...
    @Bean
    public Docket api(){
        return new Docket(DocumentationType.SWAGGER_2).select().apis(RequestHandlerSelectors.basePackage("com.stackroute")).paths((Predicate<String>) regex("/api/v1/.*")).build().apiInfo(appInfo());
    }
